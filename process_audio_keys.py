"""
Скрипт для обработки аудио с помощью обученной модели клавиш
Использование: python process_audio_keys.py input_audio.wav output_audio.wav

Использует STFT с оконным преобразованием (Hann window) и сохраняет исходную фазу
для лучшего качества восстановленного звука.
"""
...
def main():
    if len(sys.argv) < 3:
        print("="*70)
        print("Обработка аудио с помощью модели распознавания клавиш (STFT)")
        print("="*70)
        print("\nИспользование:")
        print("  python process_audio_keys.py <input_file> <output_file> [--debug] [--ref <reference_file>]")
        print("\nПримеры:")
        print("  python process_audio_keys.py raw.wav output.wav")
        print("  python process_audio_keys.py raw.wav output.wav --debug")
        print("  python process_audio_keys.py raw.wav output.wav --debug --ref processed.wav")
        print("\nТехнология: STFT с окном Ханна + сохранение исходной фазы")
        print("Поддерживаемые форматы: WAV, MP3, FLAC, OGG и т.д.")
        print("="*70)
        sys.exit(1)
    ...
    print("="*70)
    print("Обработка аудио клавиш моделью (STFT)")
    if debug:
