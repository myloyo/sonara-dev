"""
Процессор для обработки аудио акустической гитары с помощью улучшенной U-Net модели.

Алгоритм:
- Overlap-add обработка для плавных переходов
- Spectral smoothing для уменьшения шумов (мягкий для сохранения деталей)
- Spectral gating для подавления тихих частот (щадящий режим)
- Частотная коррекция для акустической гитары (сохранение воздушности)
"""
...
from app.models.ag.config import MODEL_PATH, SAMPLE_RATE, POST_PROCESSING
...
def frequency_weighting(magnitude, low_freq_boost=1.15, high_freq_attenuation=0.92):
    """
    Частотная коррекция для акустической гитары.
    Более мягкая обработка для сохранения естественных обертонов и воздушности.
    """
    freq_bins = magnitude.shape[0]
    freq_curve = np.ones(freq_bins)
    
    # Низкие частоты (0-0.25) - умеренное усиление для тела звука
    for i in range(int(freq_bins * 0.25)):
        freq_curve[i] = 1.0 + (low_freq_boost - 1.0) * (1 - i / (freq_bins * 0.25))
    
    # Средние частоты (0.25-0.5) - легкое усиление для присутствия
    for i in range(int(freq_bins * 0.25), int(freq_bins * 0.5)):
        norm_pos = (i - freq_bins * 0.25) / (freq_bins * 0.25)
        freq_curve[i] = low_freq_boost + (1.0 - low_freq_boost) * norm_pos
    
    # Высокие частоты (0.7-1.0) - очень мягкое ослабление для сохранения воздушности
    for i in range(int(freq_bins * 0.7), freq_bins):
        norm_pos = (i - freq_bins * 0.7) / (freq_bins * 0.3)
...
def load_model():
    ...
    model = ImprovedUNetSeparator(
        input_size=POST_PROCESSING.get("input_size", 1025),
        base_channels=32,
        dropout_rate=0.1
    ).to(device)
...
def process_audio(model, input_path, output_path, chunk_size=15, reference_path=None):
    ...
    Применяет мягкий spectral smoothing и gating для акустической гитары.
    """
    ...
    # Параметры пост-процессинга
    smoothing_factor = POST_PROCESSING.get("smoothing_factor", 0.15)
    gating_threshold = POST_PROCESSING.get("gating_threshold", 0.04)
    gating_floor = POST_PROCESSING.get("gating_floor", 0.2)
    low_freq_boost = POST_PROCESSING.get("low_freq_boost", 1.15)
    high_freq_attenuation = POST_PROCESSING.get("high_freq_attenuation", 0.92)
    lowpass_cutoff = POST_PROCESSING.get("lowpass_cutoff", 10000)
    ...
    if len(audio) < chunk_size * SAMPLE_RATE:
        ...
        # === POST-PROCESSING: Spectral cleanup (мягкий для акустической гитары) ===
        print("  Применяю spectral smoothing (мягкий)...")
        output_mag = spectral_smoothing(output_mag, smoothing_factor=smoothing_factor)
        
        print("  Применяю spectral gating (щадящий)...")
        output_mag = spectral_gating(output_mag, threshold=gating_threshold, floor=gating_floor)
        
        print("  Применяю частотную коррекцию...")
        output_mag = frequency_weighting(output_mag, low_freq_boost=low_freq_boost, high_freq_attenuation=high_freq_attenuation)
        # ============================================================================
        ...
        # === Фильтр для акустической гитары: low-pass 14kHz для воздушности ===
        nyquist = sr / 2
...
def process_file(input_path: str, output_path: str, chunk_size: int = 15):
    """
    Основной интерфейс для обработки аудиофайла.
    
    Args:
        input_path: Путь к входному аудиофайлу
        output_path: Путь для сохранения результата
        chunk_size: Размер чанка в секундах для обработки
    """
    model = load_model()
    process_audio(model, input_path, output_path, chunk_size=chunk_size)
